...
class FASA(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        q_local = self.q(x)
        # print(self.dim_head)
        q = q_local.reshape(b, -1, self.dim_head, h * w)
        q = q.transpose(-1, -2)
        q = q.contiguous()  # (b m (h w) d)
        k, v = self.kv(self.pool(x)).reshape(b, 2, -1, self.dim_head, H * W).permute(1, 0, 2, 4,
                                                                                     3).contiguous()  # (b m (H W) d)
...
class ConvFFN(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, kernel_size, stride, out_channels):
        ...
        self.dwconv = get_conv2d(hidden_channels, hidden_channels, kernel_size, stride, kernel_size // 2, 1,
    ...
    def __init__(self, in_chans, num_classes, embed_dims: List[int], depths: List[int], kernel_sizes: List[int],
        ...
        for i_layer in range(self.num_layers):
            if i_layer != self.num_layers - 1:
                layer = FATlayer(depths[i_layer], embed_dims[i_layer], embed_dims[i_layer + 1], kernel_sizes[i_layer],
                                 num_heads[i_layer],
                ...
                layer = FATlayer(depths[i_layer], embed_dims[i_layer], embed_dims[i_layer], kernel_sizes[i_layer],
                                 num_heads[i_layer],
                                 window_sizes[i_layer], mlp_kernel_sizes[i_layer], mlp_ratios[i_layer],
